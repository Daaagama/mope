...
def planing_matrix_interaction_effect(n, m, x_range, y_max, y_min):
...
def check(X, Y, B, n, m, x_range, y_max, y_min, norm=False):
    ...
    if Gp < cohren_cr_table:
        ...
        m += 1
        with_interaction_effect(n, m, x_range, y_max, y_min)
...
def with_interaction_effect(n, m, x_range, y_max, y_min):
    X, Y, X_norm = planing_matrix_interaction_effect(n, m, x_range, y_max, y_min)

    y_aver = [round(sum(i) / len(i), 3) for i in Y]

    B_norm = find_coef(X_norm, y_aver, norm=True)

    return check(X_norm, Y, B_norm, n, m, x_range, y_max, y_min, norm=True)


def planning_matrix_linear(n, m, x_range, y_max, y_min):
...
def linear(n, m, x_range, y_max, y_min):
    f1 = m - 1
    f2 = n
    f3 = f1 * f2
    q = 0.05

    x, y, x_norm = planning_matrix_linear(n, m, x_range, y_max, y_min)
    ...
    if Gp < cohren_cr_table:
        ...
        m += 1
        linear(n, m, x_range, y_max, y_min)
...
def main(n, m, k):
    x_range = [[15, 45], [-35, 15], [-35, -5]]
    for i in x_range:
        for j in range(len(i)):
            i[j] /= k
    if x_range[0][0] == 15:
        print("Дані згідно варіанту")
    else:
        print("Дані поділенні на коефіцієнт \"{0}\"".format(k))
    print(x_range)
    y_max = 200 + int(sum([x[1] for x in x_range]) / 3)
    y_min = 200 + int(sum([x[0] for x in x_range]) / 3)
    main_1 = linear(n, m, x_range, y_max, y_min)
    if not main_1:
        interaction_effect = with_interaction_effect(n, m, x_range, y_max, y_min)
