...
def plan_matrix(n, m, x_range, y_max, y_min):
    y = np.zeros(shape=(n, m))
    for i in range(n):
        for j in range(m):
            y[i][j] = random.randint(y_max, y_min)
...
def main(n, m, k):
    ...
    if x_range[0][0] == -30:
        ...
        print("Дані помноженні на коефіцієнт \"{0}\"".format(k))
    print(x_range)
    x_aver_max = (min(x_range[0]) + min(x_range[1]) + min(x_range[2])) / 3
    x_aver_min = (max(x_range[0]) + max(x_range[1]) + max(x_range[2])) / 3
    ...
    x, y, x_norm = plan_matrix(n, m, x_range, y_max, y_min)
    ...
    if F_p < f_t:
        ...
        print("Перезапускаємо програму, при цьому змінивши дані\n")
        for i in final_k:
            k *= i
        main(n, m, k)
...
if __name__ == '__main__':
    main(4, 4, 1)  # 124 стрічка ми реалізуємо функцію main та передаємо їй 2 аргументи: n і m
